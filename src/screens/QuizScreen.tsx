...
const QuizScreen: React.FC = () => {
  ...
  const skillPageContent = "This is a demo skill page content about React and TypeScript."; // Mock content

  useEffect(() => {
    const loadQuestions = async () => {
      try {
        const fetchedQuestions = await fetchQuizQuestions(skillPageContent, 5);
  ...
  return (
    <QuizScreenLayout
      header={<QuizHeader title={`Question ${currentQuestionIndex + 1}`} />}
      progressBar={
        <QuizProgressBar
      ...
      question={<QuizQuestionContainer question={currentQuestion.question} />}
      options={
      ...
      footer={
        isAnswered ? (
          <QuizFooterButton
            label="Next"
            onPress={handleNext}
            disabled={isChecking || currentQuestionIndex === questions.length - 1}
          />
        ) : (
          <QuizFooterButton
            label="Check"
