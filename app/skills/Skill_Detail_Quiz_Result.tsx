...
import { calculatePercentage, passed } from "@/lib/quiz-logic";

export default function QuizResultScreen() {
    const router = useRouter();
    const params = useLocalSearchParams();
    const score = Number((params as any)?.score ?? 0);
    const total = Number((params as any)?.total ?? 0);
    const percent = calculatePercentage(score, total);
    const isPassed = passed(score, total, 0.7);
